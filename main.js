...
let routineState = {
    ...
    readyClientsForRoutineRun: [], // Senders prontos no momento da inicialização da rotina (lista isolada)
    currentLiveReadyClients: [], // **NOVO**: Senders prontos em tempo real (para persistência e UI)
...
async function updateAndEmitClientStatus(clientInstance, status, qrData = null, phoneNumber = 'N/A', isInitialRestore = false, targetSocket = null) {
    ...
    emitLog(`Sender ${clientId}: ${status}` + (phoneNumber !== 'N/A' ? ` (${phoneNumber})` : ''), 'info');
}

function createClient(clientId, initialClientData = {}) {
    console.log(`[BACKEND] Tentando criar/inicializar Sender ${clientId}...`);
    ...
    client.on('qr', (qr) => {
        qrcode.toDataURL(qr, (err, url) => {
            if (err) {
                console.error(`Erro ao gerar QR Code para Sender ${clientId}:`, err);
    ...
    client.on('ready', async () => {
        ...
        try {
            ...
            // AQUI: Define _isReadyCustom como true quando o Sender está pronto
            client._isReadyCustom = true;
            updateAndEmitClientStatus(client, 'Pronto', null, phoneNumber, false);
        } catch (error) {
            console.error(`[BACKEND] Erro ao obter telefone do Sender ${clientId} no evento 'ready':`, error);
        ...
        console.log(`[BACKEND DEBUG] Sender ${clientId} está PRONTO (via evento 'ready')! Emitting update.`);
        updateAndEmitReadyClients(); // Chama aqui para atualizar a lista de Senders prontos
    });

    client.on('authenticated', () => {
        updateAndEmitClientStatus(client, 'Autenticado', null, client.lastKnownPhoneNumber, false);
        console.log(`[BACKEND DEBUG] Sender ${clientId} está AUTENTICADO! Emitting update.`);
    });
    ...
    client.on('auth_failure', (msg) => {
        ...
        console.log(`[BACKEND DEBUG] Sender ${clientId} FALHA AUTENTICAÇÃO! Emitting update.`);
    });
    ...
    client.on('disconnected', (reason) => {
        ...
        console.log(`[BACKEND DEBUG] Sender ${clientId} DESCONECTADO! Emitting update.`);
        updateAndEmitReadyClients(); // Chama aqui para atualizar a lista de Senders prontos
    });

    client.initialize().then(async () => {
        console.log(`[BACKEND DEBUG] Sender ${clientId} inicializou com sucesso.`);
        ...
        if (client.isReady) { // Esta é a propriedade do whatsapp-web.js
            ...
            try {
                ...
                console.error(`[BACKEND] Erro ao obter telefone do Sender ${clientId} após inicialização:`, error);
                phoneNumber = client.lastKnownPhoneNumber || 'N/A'; // Usa o valor já conhecido se houver erro
            }
            // AQUI: Define _isReadyCustom como true se o Sender já está pronto após inicialização
            client._isReadyCustom = true;
            console.log(`[BACKEND DEBUG] Sender ${clientId} detectado como PRONTO (via isReady check após initialize)! Forçando update.`);
            updateAndEmitClientStatus(client, 'Pronto', null, phoneNumber, false);
            updateAndEmitReadyClients(); // Chama aqui para atualizar a lista de Senders prontos
        } else {
             console.log(`[BACKEND DEBUG] Sender ${clientId} não está pronto imediatamente após a inicialização. Status conhecido: ${client.lastKnownStatus}`);
        ...
        console.error(`[BACKEND] Erro ao inicializar Sender ${clientId}:`, err);
...
function updateAndEmitReadyClients() {
    console.log('[BACKEND - DEBUG] --- Iniciando verificação de Senders prontos ---');
    const allClients = Array.from(clients.values());
    
    allClients.forEach(c => {
        // AQUI: Agora estamos logando nossa flag customizada
        console.log(`[BACKEND - DEBUG] Sender ${c.options.authStrategy.clientId}: _isReadyCustom=${c._isReadyCustom}, lastKnownPhoneNumber=${c.lastKnownPhoneNumber}`);
    });
    ...
    console.log(`[BACKEND] Senders Prontos Emitidos e Salvos: ${readyClients.length}`);
    console.log('[BACKEND - DEBUG] --- Verificação de Senders prontos finalizada ---');
}
...
async function removeClient(clientId) {
    ...
    if (client) {
        try {
            ...
            io.emit('clientStatusUpdate', { id: clientId, status: 'Removido', name: `Sender ${clientId}` });
            emitLog(`Sender ${clientId} e seus dados de sessão foram removidos.`, 'success');
            updateAndEmitReadyClients();
        } catch (error) {
            console.error(`[BACKEND] Erro ao remover Sender ${clientId}:`, error);
            emitLog(`Erro ao remover Sender ${clientId}: ${error.message}`, 'error');
        }
    }
}
...
async function processNextMessage() {
    ...
    if (readyClientsIsolated.length === 0) {
        emitLog('ERRO: Nenhum Sender "Pronto" disponível para iniciar a rotina (lista isolada vazia). Parando.', 'error');
        stopRoutineWrapper('no_clients_available');
        return;
    }

    // Se um Sender específico foi selecionado no início, usa ele
    if (routineState.routineConfig && routineState.routineConfig.selectedClientId) {
        ...
        if (selectedClient && selectedClient._isReadyCustom) {
            ...
            emitLog(`O Sender selecionado para a rotina (${routineState.routineConfig.selectedClientId}) não está mais pronto ou disponível. Parando rotina.`, 'error');
        ...
        if (initialClient && initialClient._isReadyCustom) {
            ...
            emitLog(`AVISO: Sender designado ${initialClientId} não está pronto ou indisponível. Procurando um substituto...`, 'warning');
            
            let foundFallback = false;
            // Tenta encontrar um substituto na lista de Senders isolados
            for (let i = 1; i < readyClientsIsolated.length; i++) {
                ...
                if (fallbackClient && fallbackClient._isReadyCustom) {
                    ...
                    foundFallback = true;
                    emitLog(`INFO: Sender substituto ${fallbackClientId} encontrado. Usando-o para enviar a mensagem atual.`, 'info');
                    break; // Sai do loop de busca
                }
            }

            if (!foundFallback) {
                emitLog('ERRO CRÍTICO: Nenhum Sender "Pronto" foi encontrado para assumir o envio. Parando a rotina.', 'error');
                stopRoutineWrapper('no_clients_available');
                return;
            }
        }
    ...
    if (!clientToSend) {
        emitLog('ERRO INESPERADO: Não foi possível selecionar um Sender para o envio. Parando a rotina.', 'error');
        stopRoutineWrapper('no_clients_available');
        return;
    }

    emitLog(`[${routineState.currentIndex + 1}/${routineState.totalContacts}] Tentando enviar (Sender ${clientToSend.options.authStrategy.clientId}) para ${contact.nome} (${contact.numero})...`, 'info');
    ...
    try {
        ...
        console.error(`[BACKEND] Erro ao enviar mensagem para ${contact.nome} com Sender ${clientToSend.options.authStrategy.clientId}:`, error);
        errorMessage = error.message;
    }

    if (success) {
        emitLog(`Sucesso: Sender ${clientToSend.options.authStrategy.clientId} enviou para ${contact.nome} (${contact.nome})`, 'success');
...
function startRoutineWrapper(contacts, messages, selectedClientId, minDelay, maxDelay) {
    ...
    if (readyClientsNow.length === 0) {
        emitLog('Não há Senders WhatsApp prontos para iniciar a rotina. Por favor, conecte-se.', 'error');
        return;
    }

    // Isola os Senders que estão prontos no momento de iniciar a rotina (snapshot)
    ...
    routineState.contactsList = contacts.map(contact => {
        ...
        if (!numero.startsWith('55')) {
            const normalizedNumero = '55'+numero;
            numero=normalizedNumero;
            emitLog(`Número ${contact.nome} normalizado para ${numero} (adicionado 55).`, 'info');
        }
        if (numero.length == 13 && numero.substring(4, 5) == '9') {
            const normalizedNumero2 = numero.substring(0, 4) + numero.substring(5);
            numero=normalizedNumero2;
            emitLog(`Número ${contact.nome} normalizado para ${numero} (removido '9').`, 'info');
        }
    ...
    routineState.routineConfig = { // Salva a configuração atual da rotina
        ...
        selectedClientId: selectedClientId // Armazena o ID do Sender selecionado, se houver
    };
...
function stopRoutineWrapper(reason = 'manual') {
    ...
    if (reason === 'manual') {
        ...
        finalMessage = 'Rotina parada: Sender WhatsApp indisponível.';
    } else if (reason === 'no_clients_available') {
        finalMessage = 'Rotina parada: Nenhum Sender WhatsApp disponível na lista isolada.';
    }
...
app.post('/api/set-num-clients', async (req, res) => {
    ...
    console.log(`[BACKEND] Recebida requisição para definir ${numClients} Senders.`);

    if (typeof numClients !== 'number' || numClients < 1 || numClients > 10) {
        return res.status(400).json({ success: false, message: 'Número de Senders inválido. Deve ser entre 1 e 10.' });
    }
    ...
    for (const clientId of clientsToRemove) {
        console.log(`[BACKEND] Removendo Sender ${clientId}`);
    ...
    for (let i = 1; i <= numClients; i++) {
        if (!clients.has(i)) {
            console.log(`[BACKEND] Criando novo Sender ${i}`);
    ...
    const allClientsDataToEmit = Array.from(clients.values()).map(async (clientInstance) => {
        ...
        if (clientInstance.isReady) { // Esta é a propriedade do whatsapp-web.js
            ...
            try {
                ...
                console.error(`[BACKEND] Erro ao obter telefone do Sender ${clientInstance.options.authStrategy.clientId} após set-num-clients:`, error);
             ...
             if (currentStatus === 'QR_CODE' && !qrData) {
                 try {
                    ...
                    clientInstance.lastQrData = qrData;
                     ...
                     console.error(`[BACKEND] Erro ao re-gerar QR para Sender ${clientInstance.options.authStrategy.clientId} após set-num-clients:`, qrError.message);
                     qrData = null;
                 }
             }
        ...
        return {
            ...
            name: `Sender ${clientInstance.options.authStrategy.clientId}`,
            qr: qrData,
            phoneNumber: phoneNumber,
    ...
    Promise.all(allClientsDataToEmit).then(data => {
        ...
        console.log(`[BACKEND] Status de todos os Senders (${data.length}) re-emitidos após "Definir Senders".`);
    }).catch(error => {
        console.error(`[BACKEND] Erro ao re-emitir status dos Senders após "Definir Senders":`, error);
    });

    res.json({ success: true, message: `Número de Senders definido para ${numClients}.` });
});

app.get('/api/get-num-clients', (req, res) => {
    console.log(`[BACKEND] Requisição para obter número de Senders. Senders atuais: ${clients.size}`);
...
app.post('/api/reauthenticate', async (req, res) => {
    ...
    if (!client) {
        return res.status(404).json({ success: false, message: `Sender ${clientId} não encontrado.` });
    }

    try {
        emitLog(`Solicitando reautenticação para Sender ${clientId}...`, 'info');
        await client.destroy();
        clients.delete(clientId);
        const sessionPath = path.join(SESSION_DIR, String(clientId));
        if (fs.existsSync(sessionPath)) {
        ...
        res.json({ success: true, message: `Reautenticação solicitada para Sender ${clientId}. Um novo QR Code pode ser gerado.` });
    }
    catch (error) {
        console.error(`[BACKEND] Erro ao tentar reautenticar Sender ${clientId}:`, error);
        res.status(500).json({ success: false, message: `Erro ao reautenticar Sender ${clientId}: ${error.message}` });
    }
});
...
io.on('connection', (socket) => {
    ...
    const clientPromises = Array.from(clients.values()).map(async (clientInstance) => {
        ...
        if (clientInstance.isReady) { // Esta é a propriedade do whatsapp-web.js
            ...
            try {
                ...
                console.error(`[BACKEND] Erro ao obter telefone do Sender ${clientInstance.options.authStrategy.clientId} na conexão do socket (isReady):`, error);
                phoneNumber = clientInstance.lastKnownPhoneNumber || 'N/A';
            }
            isInitial = false;
        } else {
    ...
    Promise.all(clientPromises).then(initialClientsData => {
        console.log(`[BACKEND - SOCKET.IO] Enviando ${initialClientsData.length} status de Senders para o socket recém-conectado.`);
        ...
        const requestedClientsDataPromises = Array.from(clients.values()).map(async (clientInstance) => {
            ...
            if (clientInstance.isReady) { // Esta é a propriedade do whatsapp-web.js
                ...
                try {
                    ...
                    console.error(`[BACKEND] Erro ao obter telefone do Sender ${clientInstance.options.authStrategy.clientId} na requisição (isReady):`, error);
                    phoneNumber = clientInstance.lastKnownPhoneNumber || 'N/A';
                }
                isInitial = false;
            } else {
...
async function cleanupAndExit(exitCode) {
    ...
    emitLog('Servidor encerrando. Iniciando limpeza de Senders WhatsApp...', 'warning');
    ...
    for (const [clientId, client] of clients.entries()) {
        if (client && typeof client.destroy === 'function') {
            console.log(`[BACKEND] Destruindo Sender ${clientId}...`);
            emitLog(`Destruindo instância do Sender ${clientId}...`, 'info');
            destroyPromises.push(
                client.destroy()
                      .then(() => {
                          clients.delete(clientId);
                          console.log(`[BACKEND] Sender ${clientId} destruído com sucesso.`);
                      })
                      .catch(err => console.error(`[BACKEND] Erro ao destruir Sender ${clientId}:`, err))
            );
        }
    }

    await Promise.all(destroyPromises);
    console.log('[BACKEND] Todos os Senders WhatsApp tentaram ser destruídos.');
    emitLog('Limpeza de Senders WhatsApp concluída.', 'success');
...
server.listen(PORT, async () => {
    ...
    if (existingSessions.length === 0) {
        console.log('[BACKEND] Nenhuma sessão existente encontrada, criando Sender 1.');
        createClient(1);
    } else {
        console.log(`[BACKEND] Sessões existentes encontradas: ${existingSessions.join(', ')}. Restaurando Senders...`);
            ...
            if (!isNaN(id)) {
                return new Promise(resolve => {
                    ...
                    const timeout = setTimeout(() => {
                        console.log(`[BACKEND WARNING] Sender ${id} não atingiu 'ready' ou 'error' em tempo. Resolvendo com status atual.`);
                        resolve();
        ...
        console.log('[BACKEND] Todos os Senders existentes foram processados na inicialização do servidor.');
        // Após a inicialização de todos os Senders, atualiza a lista live e salva
        updateAndEmitReadyClients();
    }
});